...
GO
--HistoryID [int] IDENTITY(1,1) NOT NULL: Khóa chính, tự động tăng, định danh duy nhất cho mỗi bản ghi lịch sử.
--ProductID [int] NOT NULL: Tham chiếu đến sản phẩm bị thay đổi.
--OrderID [int] NULL: Tham chiếu đến đơn hàng liên quan (nếu có), ví dụ: nhập/xuất kho theo đơn hàng.
--ChangeType [int] NULL: Loại thay đổi (ví dụ: 1 = nhập kho, 2 = xuất kho, 3 = điều chỉnh).
--QuantityChanged [int] NOT NULL: Số lượng thay đổi (có thể âm hoặc dương).
--PreviousQuantity [int] NOT NULL: Số lượng trước khi thay đổi.
--NewQuantity [int] NOT NULL: Số lượng sau khi thay đổi.
--ChangeDate [datetime] NULL DEFAULT (GETDATE()): Thời gian xảy ra thay đổi.
--UserID [int] NOT NULL: Người thực hiện thay đổi.
--Note [nvarchar] NULL: Ghi chú về lý do thay đổi.
--CreatedAt [datetime] NOT NULL DEFAULT (GETDATE()), UpdatedAt [datetime] NULL: Theo dõi thời gian tạo và cập nhật bản ghi.
...
GO
ALTER TABLE [dbo].[Products] WITH CHECK ADD FOREIGN KEY([SupplierID]) REFERENCES [dbo].[Suppliers] ([SupplierID])
GO
